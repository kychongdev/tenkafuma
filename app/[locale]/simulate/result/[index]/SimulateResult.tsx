...
export function SimulateResult() {
  ...
  const totalDamage = result
    ? result.baseResult.damageLog1.reduce((acc, cur) => acc + cur.damage, 0)
    : 0;
  const totalDamage2 = result
    ? result.baseResult.damageLog2.reduce((acc, cur) => acc + cur.damage, 0)
    : 0;
  const totalDamage3 = result
    ? result.baseResult.damageLog3.reduce((acc, cur) => acc + cur.damage, 0)
    : 0;
  const totalDamage4 = result
    ? result.baseResult.damageLog4.reduce((acc, cur) => acc + cur.damage, 0)
    : 0;
  const totalDamage5 = result
    ? result.baseResult.damageLog5.reduce((acc, cur) => acc + cur.damage, 0)
    : 0;
  const total =
    totalDamage + totalDamage2 + totalDamage3 + totalDamage4 + totalDamage5;
  const baseEachTurnTotal =
    baseEachTurnResult.length > 0
      ? baseEachTurnResult.map((value) => {
  ...
  return (
    <div className="w-full mx-auto md:max-w-[600px] font-[family-name:var(--font-geist-sans)] p-2">
      {result ? <DisplaySelect team={result.team} /> : null}

      <Card className="p-1">
        <Collapsible>
          <div className="flex items-center justify-between space-x-4 px-1">
            {result ? parseBond(result.team) : ""}
      ...
      {result
        ? result.analysis.map((value, i) => {
            ...
            const dmg1 = value.damageLog1.reduce(
              (acc, cur) => acc + cur.damage,
              0,
            );
            const dmg2 = value.damageLog2.reduce(
              (acc, cur) => acc + cur.damage,
              0,
            );
            const dmg3 = value.damageLog3.reduce(
              (acc, cur) => acc + cur.damage,
              0,
            );
            const dmg4 = value.damageLog4.reduce(
              (acc, cur) => acc + cur.damage,
              0,
            );
            const dmg5 = value.damageLog5.reduce(
              (acc, cur) => acc + cur.damage,
              0,
            );
