...
import { healHpBasicDamage } from "./calculations/healHpBasicDamage";
...
export function basicHpHealAllAllies(
  G: GameState,
  oG: GameState,
  value: number,
  attacker: Target,
  ...
  for (let i = 0; i < 5; i++) {
    const dmg = healHpBasicDamage(G, oG, value, attacker, i);
    healTarget(G, dmg, i);
    writeToHealLog(G, attacker, i, dmg, DamageType.BASIC, ca);
  }
}

export function basicHealAllAllies(
  G: GameState,
  oG: GameState,
  value: number,
  attacker: Target,
  ...
  for (let i = 0; i < 5; i++) {
    const dmg = healBasicDamage(G, oG, value, attacker, i);
    healTarget(G, dmg, i);
    writeToHealLog(G, attacker, i, dmg, DamageType.BASIC, ca);
  }
}
