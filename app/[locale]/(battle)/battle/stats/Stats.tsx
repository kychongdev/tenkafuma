...
import { DamageType } from '@/types/Skill';
...
export default function Stats() {
  ...
  const totalDamage = useGameState((state) =>
    state.damage_log_1.reduce((acc, log) => acc + log.damage, 0),
  );
  ...
  const totalDamage2 = useGameState((state) =>
    state.damage_log_2.reduce((acc, log) => acc + log.damage, 0),
  );

  const triggerDamage2 = useGameState((state) =>
    state.damage_log_2.reduce((acc, log) => {
      if (log.type === DamageType.TRIGGER) {
        return acc + log.damage;
      }
      return acc;
  ...
  const totalDamage3 = useGameState((state) =>
    state.damage_log_3.reduce((acc, log) => acc + log.damage, 0),
  );

  const triggerDamage3 = useGameState((state) =>
    state.damage_log_3.reduce((acc, log) => {
      if (log.type === DamageType.TRIGGER) {
        return acc + log.damage;
      }
      return acc;
  ...
  const totalDamage4 = useGameState((state) =>
    state.damage_log_4.reduce((acc, log) => acc + log.damage, 0),
  );

  const triggerDamage4 = useGameState((state) =>
    state.damage_log_4.reduce((acc, log) => {
      if (log.type === DamageType.TRIGGER) {
        return acc + log.damage;
      }
      return acc;
  ...
  const totalDamage5 = useGameState((state) =>
    state.damage_log_5.reduce((acc, log) => acc + log.damage, 0),
  );

  const triggerDamage5 = useGameState((state) =>
    state.damage_log_5.reduce((acc, log) => {
      if (log.type === DamageType.TRIGGER) {
        return acc + log.damage;
      }
      return acc;
