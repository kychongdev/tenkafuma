...
export function enemyDealBasicDmgToHighestHp(
  ...
  const dmg = basicDamage(G, oG, value, attacker, defender, false);
  dealDamage(G, dmg, defender, isTrueDamage);
  writeBattleLog(G, attacker, defender, dmg, damageType, action);
}

export function enemyDealBasicDmgToAllAllies(
  G: GameState,
  oG: GameState,
  value: number,
  attacker: Target,
  ...
  for (let i = 0; i < 5; i++) {
    const dmg = basicDamage(G, oG, value, attacker, i, false);
...
function writeBattleLog(
  ...
  gameState.enemyBattleLog.push(
    `[${parseActionName(action)}] ${attackerName} 對 ${defenderName} 造成 ${formatNumber(res1)} 傷害 (${parseDamageTypeName(damageType)})`,
  );
}
